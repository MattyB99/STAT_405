...
a <- data.frame(Year=c(seq(from=2010, to=2020, by=1), "Total"),
...
b <- data.frame(Top_Violations= c("NO PARK/STREET CLEAN", "METER EXP", "RED ZONE"), Count=c(2389114, 1634877, 635504), fine_amount=c(73,63,93))# %>%knitr::kable()
...
list(a,d)%>% knitr::kable()
list(b,c)%>% knitr::kable()
...
ggplot(map_data("state")%>%left_join(Parking_Citations%>%group_by(RP.State.Plate)%>%summarise(count = n())%>%left_join(read.csv('statename.csv'))%>%filter(!is.na(region), region != "california"))) + aes(long, lat, group = group) +
  geom_polygon(aes(fill = count), color = "white") + scale_fill_viridis_c(option = "C") + theme_void() + theme(legend.position = c(0.1,0.17)) + 
  labs(fill = "Number of Citations", title = "Citations Issued by State of Vehicle Registration")
```
  It appears that for citations written for vehicles from outside of California, the states that received the most are either nearby states-- Arizona, Nevada, and Oregon --or highly populated states, such as Texas and Florida.
```{r, echo=FALSE, message=FALSE, warning=FALSE}
...
crime500Map <- ggmap(map) +
  ...
  scale_fill_gradient(low = "red", high = "yellow") +
  scale_alpha(range = c(0, 1), guide = FALSE) + 
  theme(axis.text.x = element_text(hjust  = 1, angle = 45)) + labs(title = "Less Severe Crime")
...
badCrimeMap <- ggmap(map) +
  ...
  scale_fill_gradient(low = "red", high = "yellow") +
  scale_alpha(range = c(0, 1), guide = FALSE) + 
  theme(axis.text.x = element_text(hjust  = 1, angle = 45)) + labs(title = "Severe Crime")
...
##### Does weather or crime affect the parking convictions overall?
  In our investigation of how parking citations are distributed across the city of LA, we wondered how crime data and potentially weather data would fit in with our parking citations data. We used 2 SQL databases of both LA weather and LA crime to compare with our initial data. By using a heatmap on the city of LA with out data we can see different concentrations of crime, weather, etc., and in conjunction with our initial dataset we can see if there are any correlations between crime, weather, and our parking citations. By plotting the heatmaps of the data, we can see the hotspots of parking citations etc. shown below.
```{r, echo=FALSE, warning=FALSE, message=FALSE}

grid.newpage()
ly <- grid.layout(1, 2)
pushViewport(viewport(layout = ly))
print(crimeMap, vp = viewport(layout.pos.row = 1, layout.pos.col = 1))
print(weatherMap, vp = viewport(layout.pos.row = 1, layout.pos.col = 2))

grid.newpage()
ly <- grid.layout(1,1)
pushViewport(viewport(layout = ly))
print(overallMap, vp = viewport(layout.pos.row = 1, layout.pos.col = 1))
```
