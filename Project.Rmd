...
```
  ##past tense
   Alex, Anushi, Matt and Griffin all come from different backrouns and majors. So in disucssions for which dataset we wanted, we dabbled on a whole range of topics like  public health and New York CIty subway data. WHen we came down to our decision, we decided to take on data from another large metropolis, but this time in sunny LA. In Los Angeles, the beautiful beaches, actors, and hollywood homes are magical for people who visit. Yet, despite this LA, traffic is the worst and parking in tourist areas can be a nightmare. So, wherever there is parking, there are rules and parking violations. Unironically, our dataset, Parking_Citations, contains all the details of the 8,502,692 parking violations in Los Angeles. That means we have access to the records that account for 591,316,252 in a 10 year span. With so much money at stake, of course the city of Los Angeles wants to keep track of these records electronically. While that data is imperfect and skewed towards 2018, Parking_Citations presented raw data into our hands. 
...
When we began our analysis, we had to get some idea of what we were dealing with. With no instructions, We first boiled down the data into a more understandble form. The data gave us a good amount of information, and it also gave us a good starting point. "Parking Citations by Year" revealed the sheer volume of the, although it was showed a skewed distrubution over 11 year span. From 2010 to 2014 there were less than 40 parking citations recorded. In 2014 that number increased to 34,000. After that, we saw a drastic 44 percent change into 2015, when 1,541,535 parking citations were recorded. The count peaks at 2,154,321 citations in 2017. This peak accentuates both the steep rise leading up to 2017 and also the suprisingly steep decline afterwards. While this graph gave us a good handle on the numbers, there were questionsunanswered about the collection of the data.The jump drastic increase from 2014-2015 may be explained by an attempt by the City of Los Angeles to digitize their citations. But that explanation is contradicted by the equally sharp decline after 2017. This left the question of how many citations did they actually collect? The graph didn't tell us exactly, but it gives us a range of values where the true value is close. If anything, this graph highlighted the magnitude of the citaitons in LA.

```{r}
df <- data.frame(Year=c(seq(from=2010, to=2020, by=1), "Total"),
         "Dollars_Collected" =c(730, 630, 2276, 3062, 2371400, 107172874, 130124585,151608139,124738695,75291586,2275, sum(df$Dollars_Collected)))
y
df <- as.data.frame(df)
df
knitr::kable(df)
