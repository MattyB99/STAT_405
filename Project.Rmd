...
Parking_Citations[1:1000, 1:22] %>% 
  ...
  scale_fill_manual(values = c("Black", "Blue", "Brown", "Gray", "Red", "White"))
```
...
holiday_names <- data.frame(date= c("02/14", "03/17", "06/04", "10/31", "11/22", "11/23", "11/24", "11/26", "11/28", "12/25", "12/31"), 
                            name = c("Valentine's", "St. Patricks", "July 4th", "Halloween", rep("Thanksgiving", 5), "Christmas", "New Years"))

h_parking <- Parking_Citations%>%filter(Issue.Date %in% holidays)%>%
              mutate(date = format(as.Date(Issue.Date, format = "%m/%d/%Y"), format = "%m/%d"))%>%left_join(holiday_names, by = 'date')


h_parking%>%group_by(name)%>%summarise(mean = mean(Fine.amount), sum = sum(Fine.amount), count = n()) 
```
