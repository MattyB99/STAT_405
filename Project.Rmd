...
```
### Introduction  
   Alex, Anushi, Matt and Griffin all come from different backrounds and majors, so in disucssions for which dataset we wanted to analyze, we explored a wide array of topics ranging from public health data to the New York City subway data. We eventually decided to take on data from another large metropolis, but this time in sunny Los Angeles. The beautiful beaches, countless celebrities, and hollywood homes in Los Angeles make it a desireable place to live. However, LA traffic is a dealbreaker for some. Abismal traffic and poor parking compounded with a lucrative tourist industry makes driving and parking a nightmare. As the second largest city in the United States, there are over 6.4 million vehicles in the Los Angeles urbanized area^1^. Our dataset, Parking_Citations, contains all the details of nearly 10 million parking violations in Los Angeles from 2010 to the present. That means we have access to the records that account for fines totalling close to $600 million. With this data, we want to use r to break down and model the madness, using an interdiscplinary and unconvential approach to find new insights and explore new questions
...
```{r, echo=FALSE, warning=FALSE, message=FALSE, cache=TRUE}
>>>>>>> 19ddcc87eccd7f81e3a499956afe3e58977a3dc2
...
print(crimeMap, vp = viewport(layout.pos.row = 1, layout.pos.col = 3))
...
```{r holidays, echo=FALSE, fig.align="center", message=FALSE, warning=FALSE, out.width="1.15\\linewidth", out.height="1.15\\linewidth", cache=TRUE}
...
3.
“Parking Citations: Los Angeles - Open Data Portal.” Data.lacity.org, 13 Feb. 2020, data.lacity.org/A-Well-Run-City/Parking-Citations/wjz9-h9np.
