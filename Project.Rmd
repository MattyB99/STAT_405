...
##Parking_Citations%>%select(RP.State.Plate)%>%
##filter(!is.na(RP.State.Plate))%>%
##filter(RP.State.Plate!="CA")%>%
##group_by(RP.State.Plate)%>%
##  summarise(count=n())
...
ggplot(map_data("state")%>%left_join(Parking_Citations%>%group_by(RP.State.Plate)%>%
                                       summarise(count = n())%>%left_join(read.csv('statename.csv'))%>%filter(!is.na(region), region != "california"))) + 
  aes(long, lat, group = group) +
  geom_polygon(aes(fill = count), color = "white") + scale_fill_viridis_c(option = "C")
```
